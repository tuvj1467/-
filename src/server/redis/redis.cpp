...
bool Redis::subscribe(int channel)
{
    ...
    if (REDIS_ERR == redisAppendCommand(this->subcribe_context, "SUBSCRIBE %d", channel))
    {
    ...
    while (!done)
    {
        if (REDIS_ERR == redisBufferWrite(this->subcribe_context, &done))
        {
            cerr << "subscribe command failed!" << endl;
            return false;
        }
...
{
    if (REDIS_ERR == redisAppendCommand(this->subcribe_context, "UNSUBSCRIBE %d", channel))
    {
...
void Redis::observer_channel_message()
{
    ...
    while (REDIS_OK == redisGetReply(this->subcribe_context, (void **)&reply))
    {
        // 订阅收到的消息是一个带三元素的数组
